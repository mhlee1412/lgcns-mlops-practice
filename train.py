...
sys.excepthook = handle_exception  # try except없이 error에 대해 trace back 가능
warnings.filterwarnings(action="ignore")


if __name__ == "__main__":
    logger.info("Loading ata..")
    ...
    param_set = get_param_set(params=params_candidates)  # 파라미터의 순서쌍을 만듬
    ...
    logger.debug("Using mlfow to track an experiment...")
    for i, params in enumerate(param_set):
        ...
        with mlflow.start_run(run_name=f"Run {i}"):
            ...
            logger.info(f"RMSE_CV for Run {i} : {score_cv.mean()}")
